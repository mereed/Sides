...
kpay.setEventHandler((e, data) => {
  switch (e) {
    ...
    case kpay_common.eventTypes.PurchaseStarted:
      ...
      settingsStorage.setItem('kpayStatus', `To continue using Sides, please visit kzl.io/code and enter this code: ${data}`);
      break;
...
settingsStorage.onchange = evt => {
  ...
  if (evt.key === "background-image") {
    ...
    //sending other settings to device
    let data = {
      key: evt.key,
...
function restoreSettings() {
  for (let index = 0; index < settingsStorage.length; index++) {
    ...
    if (key && key != "background-image" ) { //not restoring image
      let data = {
        key: key,
